...
describe('Calendar API', () => {
  ...
  it('clears current and legacy autogenerated tentative notes when confirmed', async () => {
    const noteFormats = [
      'Auto set as tentative \u2014 six-month rollover from last calibration: 2025-10-20',
      'Tentative \u2014 based on Oct 2025 calibration',
      'Tentative \u2014 six-month rollover from 2025-10-20',
    ];

    for (const notes of noteFormats) {
  ...
  it('removes an autogenerated tentative when a manual event is created for the same job', async () => {
    ...
    expect(statements.some(sql => sql.includes('DELETE FROM calendar_events')
      && sql.includes("instr(notes, 'Auto set as tentative')"))).toBe(true);
  });
});
